...
gp.cor3 <- datSM2 %>%
  split(datSM2$group.factor) %>%  
  map(~corr.test(datSM2 %>% select(ess_total, bdi_total),
                 use = "everything",
                 method = "spearman",
                 adjust = "none",
                 alpha = 0.05,
...
gp.cor3.r <- map(gp.cor3, ~.x$r)
gp.cor3.p <- map(gp.cor3, ~.x$p)
print(x = gp.cor3.r, short = TRUE)
print(x = gp.cor3.p, short = TRUE)
