...
print(signif(model_summaries$bdi_total$tukey$group.factor, 3))  %>% gt()
...
```{r ---> TABLES}
z_table2 <- print(signif(model_summaries$bdi_total$tukey$group.factor, 3))
z_table3 <- as.data.frame(z_table2)
z_table3$row <- c("CLBP-HC", "FM-HC", 
				  "FM-CLBP")

print(signif(model_summaries$bdi_total$tukey$group.factor, 3))
z_table1 <- as_tibble(signif(model_summaries$bdi_total$tukey$group.factor, 3))
