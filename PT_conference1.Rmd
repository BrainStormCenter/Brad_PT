...
plot(dat.pain)
summary(dat.pain)
e <- dat.pain %>% select(!"group.factor")
e <- e %>% select(!"slpDist")
plot(e)
pcr <- cor(e)
corrplot(pcr)
...
## circle + colorful number
#		I LIKE THIS VERSION
corrplot(acr, order = "AOE", type = "upper", tl.pos = "d",
		 bg = "grey",
		 outline = TRUE,
		 hclust.method = "ward.D2",
		 addgrid.col = "blue",
		 col = col1(200))
corrplot(acr, add = TRUE, type = "lower", method = "number", order = "AOE",
		 diag = FALSE, tl.pos = "n", cl.pos = "n",
		 bg = "grey",
		 outline = TRUE,
		 col = col1(200))
...
corrplot(pcr, p.mat = res3$p,  insig = "blank", sig.level = 0.05) ## add all p-values

########		corrplot.mixed 		#########

corrplot.mixed(pcr2, tl.pos = "lt") ## I LIKE THIS VERSION AS A START
