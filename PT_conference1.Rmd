...
As part of the study, participants completed a variety of questionnaires, via secure RedCap research database, related to pain variables

Participants completed the Beck Depression Inventory-II (BDI-II), Epworth Sleepiness Scale, McGill-Melzack Pain Questionnaire and Pittsburg Sleep Quality Index (PSQI). A subgroup of participantas also completed the Insomnia Severity Index.
...
```{r ISIsubgroup, eval=TRUE, echo=FALSE, message=FALSE, comment=""}
...
## DATA VARIANCE
The variable slpDist has little to no variance in:  
* dat.sm  
* dat.pain (both pain groups)  
* dat.isi
...
**NOTE** I need to add a section that discusses the ISI. Anything over "5" is considered significant - I think.  
...
```{r BOXPLOT-test, eval=TRUE, echo=FALSE, message=FALSE, comment = ""}
# PERFORMING TEST
...
```{r BARPLOT-test, eval=TRUE, echo=FALSE, message=FALSE, comment = ""}
...
```{r ANOVA-rawdata, eval=TRUE, echo=FALSE, message=FALSE}
...
## POSTHOC 
```{r PostHoc1, eval=TRUE, echo=FALSE, message=FALSE}
...
```{r PostHoc2, eval=TRUE, echo=FALSE, message=FALSE}
# GetTukeys <- function(m) m$tukey
...
```{r multiplot, eval=TRUE, echo=FALSE, message=FALSE, comment = ""}
