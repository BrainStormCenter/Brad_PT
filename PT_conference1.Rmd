...
```{r ---> Color schemes, eval=TRUE, echo=FALSE, message=FALSE}
...
```{r ---> Date for files, eval=TRUE, echo=FALSE, message=FALSE}
...
```{r ---> Clean data, eval=TRUE, echo=FALSE, message=FALSE}
...
```{r ---> PSQI dataset, eval=TRUE, echo=FALSE, message=FALSE}
#		PSQI DATA
...
```{r ---> chr to num, eval=TRUE, echo=FALSE}
...
## FINAL DATASETS
All data has been updated, processed, clean and scored  
```{r ---> DATA TO ANALYZE, eval=TRUE, echo=FALSE, comment=""}
...
```{r ---> ANOVA with PostHoc, eval=TRUE, echo=FALSE, message=FALSE}
#	PostHoc DOES WORK
...
Conditional tables
```{r ---> conditional-formating, eval=TRUE, echo=FALSE, message=FALSE}
Models2 %>%
	mutate(p.value = 
		   	cell_spec((formatC(x = p.value, 
		   					   digits=3, width = 3, 
	...
	mutate_if(is.numeric, function(x) {
		cell_spec((formatC(x,digits=2, width=3, 
	...
	footnote(general = "Red = main effect for group") %>% 
...
## PostHoc 
```{r ---> POSTHOC RESULTS, eval=TRUE, echo=FALSE, message=FALSE}
...
####		THIS PUT THE PostHoc TESTS INTO A TABLE		####
select(Tukeys, -term) %>% 
...
```{r ---> GT tables ver 1,eval=FALSE, echo=FALSE}
